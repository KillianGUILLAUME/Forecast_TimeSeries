...
class LSTMPredictionModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=3, dropout=0.2):
        ...
        """ Attention mechanism (not used in forward pass currently) """
        ...
        #projection if dimension mismatch
        self.proj = nn.Linear(d_in, d_mid) if d_in != d_mid else nn.Identity()
        self.out = nn.Linear(d_mid, output_size)
    ...
    def forward(self, x):
        ...
        w = torch.softmax(e.squeeze(-1), dim=1)
        ...
        # h_0 = torch.zeros(self.num_layers * self.num_directions, x.size(0), self.lstm.hidden_size).to(x.device)
        # c_0 = torch.zeros(self.num_layers * self.num_directions, x.size(0), self.lstm.hidden_size).to(x.device)
        
        # out, (hn,cn) = self.lstm(x, (h_0, c_0))
        # hn = hn.view(self.num_layers, self.num_directions, x.size(0), self.hidden_size)
        # h_last = hn[-1].permute(1, 0, 2).reshape(x.size(0), self.num_directions * self.hidden_size)
...
class LSTMPredictor:
    ...
    def fit(self, df: pd.DataFrame):
        ...
        self.model = LSTMPredictionModel(
            input_size=len(self.feature), 
            hidden_size=self.hidden_size, 
            output_size=self.output_h, 
        ...
        best_loss, patience, bad = float('inf'), 300, 0

        lr_to_plot, loss_plot = np.zeros(self.epochs), np.zeros(self.epochs)
    ...
    def predict(self, df: pd.DataFrame) -> np.ndarray:
        ...
        self.model.eval()
        with torch.no_grad():
            # for _ in range(self.output_h):
                # pred = self.model(input)
                # preds_scaled.append(pred.squeeze(0).detach().cpu().numpy())
                # input = torch.cat((input[:, 1:, :], pred.unsqueeze(1)), dim=1)
            out_sc = self.model(input)
            preds_scaled = out_sc.detach().cpu().numpy()
        
        preds_ret = self.scaler_y.inverse_transform(preds_scaled).squeeze(0)

        last_p = df['adj_close'].iloc[-1]
        preds = last_p * np.exp(np.cumsum(preds_ret))
